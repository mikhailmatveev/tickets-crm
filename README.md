...
Все тесты выполняются в окружении `testing` и на отдельной тестовой базе, которая очищается после каждого теста

Тестовая база наполняется через `seeders` в методах `setUp()`

Выполняем прогон Unit и Feature тестов
